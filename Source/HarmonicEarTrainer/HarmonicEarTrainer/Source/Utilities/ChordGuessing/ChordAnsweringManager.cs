...
namespace HarmonicEarTrainer
{
    static class ChordAnsweringManager
    {
        ...
        public static string evaluatedChordTypeAnswer { get; set; } = ""; // para poner si la respuesta es correcta o equicovada
        static Color _evaluatedChordTypeAnswerColor; // va a cambiar dependiendo de si la respuesta fue correcta o equivocada
        public static string evaluatedChordInversionAnswer { get; set; } = ""; // para poner si la respuesta es correcta o equicovada
        ...
        {
            _submitAnswer = new Bang(new Vector2(LayoutManager.marginLeft, LayoutManager.marginTop + LayoutManager.gridRowSeparation * 18), "Submit [ enter ]", (int)BangCategory.Chord, true);
            _revealAnswer = new Bang(new Vector2(LayoutManager.marginLeft + LayoutManager.gridColumnSeparation, LayoutManager.marginTop + LayoutManager.gridRowSeparation * 18), "Reveal [ down ]", (int)BangCategory.Chord, true);
        ...
        private static void DisplayChordDescription()
        {
            ...
            //Los casos del chordType y el chordInversion son más elaborados, porque para obtener los índices de los diccionarios como integers (que es como los preciso) debo convertir a integers los valores de las variables chordType e chordInversion, alojadas en ChordPlayingManager.chord1 (como uso enums esos valores son de tipo ChordType y ChordInversion, por eso debo convertirlos).
        ...
        public static void SubmitShortcut() //función que se llama si uso el shortcut para enviar respuesta (tecla enter)
        {
            _submitAnswer.OnBangClickedWithLeftButton(); //Si uso el shortcut, quiero hacer lo mismo que si clickeara ese ese bang con click izquierdo
        }

        public static void RevealShortcut() //función que se llama si uso el shortcut para revelar acorde (tecla abajo)
        {
            _revealAnswer.OnBangClickedWithLeftButton(); //Si uso el shortcut, quiero hacer lo mismo que si clickeara ese ese bang con click izquierdo
        }
