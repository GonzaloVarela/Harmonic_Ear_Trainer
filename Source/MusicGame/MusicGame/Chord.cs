...
public class Chord
{
    ...
    bool[] isTypeEnabled = new bool[23];

    //array de bools para determinar qué inversions están habilitadas
    bool[] isInversionEnabled = new bool[4];

    public Chord()
    {
        for (int i = 0; i < isTypeEnabled.Length; i++) isTypeEnabled[i] = true; //por ahora (que no tengo opciones) comienzo la instancia del acorde con todos los tipos de acorde habilitados

        for (int i = 0; i < isInversionEnabled.Length; i++) isInversionEnabled[i] = true; //por ahora (que no tengo opciones) comienzo la instancia del acorde con todos los tipos de acorde habilitados
            ...
            case ChordType.Major:
                ...
                switch (inversion)
                {
                    ...
                    case ChordInversion.SecondInversion:
                        //Si el acorde es major y está en segunda inversión, solo puede duplicar la 5ta (el bajo).
                        _noteHighAPitchClass = _noteBassPitchClass;
                        _noteHighAOctave = Main.randomize.Next(4, 6);
    ...
    public void ArpeggioUpdate(GameTime gameTime)
    {
    ...
    int RandomizeTypeAmongEnabledTypes() //Randomiza el tipo de acorde entre los tipos de acorde que haya habilitados
    {
        ...
        {
            if (isTypeEnabled[mappedTypeNumber]) unmappedTypeNumber--; //si y solo sí el índice IsTypeEnabled por el que se está pasando está enabled, tengo que gastarme uno de los pasos
